...
\label{sec:3level}
Now we demonstrate how to design a e-level policy to incentivize
adaptive exploration and improve on the $O(T^{2/3})$ regret rate. In
\Cref{sec:llevel}, we will show how to extend this idea to achieve a
regret rate of $\tilde O(T^{1/2})$. For simplicity, we will focus on
the setting with 2 arms.
...
\swcomment{can we reuse the previous result?}
