...
$(S\cdot T_1\cdot \GdT)$ agents, where $T_1 = T^{4/7}\log^{-1/7}(T)$
and $S = 2^{10}\log(T)$. In particular, the policy partitions
the agents into $S$ groups such that each group consists of $T_1$ runs
of \ALGG with $\GdT$ rounds. In the second level, the policy performs
...
\caption{Structure of the information graph for the 3-level
  ...
  length $\GdT$. The sub-history of each red box is then aggregated and
...
\[
\reg(T) \leq (S \GdT T_1 + S T_2) \cdot 2\left(\sqrt{\frac{4\log(T)}{q_1T_1}} 
