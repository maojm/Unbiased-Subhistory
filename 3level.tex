...
The key idea is as follows. Consider the gap parameter $\Delta = |\mu_1-\mu_2|$. If it is is large, then each first-level group produces enough data to determine the best arm with high confidence, and so each agent in the upper levels chooses the best arm. If $\Delta$ is small, then due to \emph{anti-concentration} each arm can get ``lucky" within any given first-level group and appear much better to the corresponding second-level group (and therefore get explored within that group). To summarize, the middle level exploits if the gap parameter is large, and explores if it is small.
