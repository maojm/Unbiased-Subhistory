...
We will first establish four \emph{clean events} that together occur
with probability at least $1 - O(1/T)$. The first one says that every
group of \ALGG runs in the first level ensures that each arm $a$ is
pulled at least $\Omega(T_1)$ times, which follows from
Lemma~\ref{lem:t1runs} and union bound.

\begin{lemma}[Concentration of first-level number of pulls.]
...
\]
\swcomment{let's put this in appendix.}
\end{proof}
...
hypothetical reward tape tape $\cT_{s, a}$ of length $T$ for each
...
from the $j$-th cell in this arm’s tape. We now show the concentration
behavior of the mean rewards across all consecutive pulls among all
such tapes.

\begin{lemma}[Concetration of empirical means]
  ...
  $\cT_{a, s}$ is at most $\sqrt{\frac{2\log(T)}{t_2-t_1+1}}$ away
...
\begin{lemma}[Anti-concentration of empirical means]
  For any $s\in [S]$, any arm $a$, let $\tilde\mu_{a,s}$ be the
  empirical mean of the first $q_a T_1$ cells in tape $\cT_{a, s}$.
  Let $W_4^{s,a,high}$ be the event
  $\tilde\mu_{a, s} \geq \mu_a + 1/\sqrt{q_a T_1}$ and let
  $W_4^{s,a,low}$ be the event that
  $\tilde\mu_{a, s} \leq \mu_a - 1/\sqrt{q_a T_1}$.  Let $W_4$ be
  $\bigcap_a \bigcup_s (W_4^{s,a,high} \cap W_4^{s,3-a,low})$.
  \[
