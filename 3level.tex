...
the data collected in any sing first-level group can distinguish the
two arms. The proof essentially follows from the clean event $W_1$
...
that every arm $a$ has its ``lucky'' group $s_a$ in the first level,
for which the empirical mean is slightly higher than $\mu_a$, while
...
both medium and small gap, it suffices to bound the regret from the
