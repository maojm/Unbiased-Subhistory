...
simplicity, we will focus on the setting with 2 arms\niedit{, although our results can be extended to a constant number of arms $K$}. \nicomment{I think that's true, but if not, then we're not focusing on 2 arms for ``simplicity''.} \jmcomment{It is true. We only show this extension for $l$-level tho.}
...
history from the first two levels. \nicomment{This comment applies to the 2level too; why not allow the guys within the last level see each other?  Seems like this would still work but be more of a headache to prove -- would have to argue the empirical means can't move around too much in the sub-terminal levels, but it makes a big qualitative difference because now everyone sees a vanishingly small fraction of the history whereas with this modification a fraction approaching 1 of the people see a fraction approaching 1 of the history.} \jmcomment{To do it for the last level is actually easy. It would be headache to do it for the second level as we want to prove anti-concentration in the second level. On the other hand, we have the property that agents see $1/polylog$ fraction of history in our final result for $l$-level. A different point of view is that, seeing each other requires more interaction, our policy has the nice property that the interaction is very minimal except the initial greedy runs.}


They main idea for the 3-level policy is that it inserts the second level as an additional ``checkpoint'' to induce more adaptivity in the exploration. While the first level ensures that the agents explore both arms, the second level will continue to explore both arms only if the rewards of the two arms are still indistinguishable (that is, when $|\mu_1 - \mu_2| < \tilde{O}(1/\sqrt{T_1})$). In the third level, agents will pull the best arm unless $|\mu_1-\mu_2|$ is much smaller than $\tilde{O}(1/\sqrt{T_2})$, in which case pulling either arm will
incur small regret. \nicomment{Need to add intuition about why it's necessary to have a middle level to get this to happen, as opposed to making the bottom level bigger.  Is it because the bottom level will start to herd on some arm and we want to refresh that process? Also should talk about need for anti-concentration bound.} The formal guarantee of the policy is the
following.
...
\begin{lemma}[Small gap case]\label{3levelsmallcase}
  ...
  \[
    \Delta\in \left( 3\sqrt{\frac{2\log(T)}{T_2}},
      2\left(\sqrt{\frac{4\log(T)}{S q_1T_1}} +
        \sqrt{\frac{4\log(T)}{S q_2T_1}}\right) \right)
  \]
\end{lemma}
...
by $S T_G T_1 = O(T^{4/7} \log^{6/7}(T))$. In the intermediate cases of
both medium and small gaps, it suffices to bound the regret from the
first and second levels, so
\[
\reg(T) \leq (S T_G T_1 + S T_2) \cdot 2\left(\sqrt{\frac{4\log(T)}{q_1T_1}} 
...
\OMIT{By Lemma \ref{lem:greedy}, we know $\GdP \leq q_a \leq \GdT$. For the $s$-th first-level group, define $W_1^{a,s}$ to be the event that the number of arm $a$ pulls in the $s$-th first-level group is between $q_a T_1- \GdT \sqrt{T_1\log(T)}$ and $q_a T_1 + \GdT \sqrt{T_1\log(T)}$. By Chernoff bound,}
