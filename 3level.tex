...
\begin{lemma}[Concetration of empirical means in the first level]
...
\begin{lemma}[High and low deviations for the arms]
...
\begin{lemma}
  ...
  $W_3$ holds: for all $a\in \{1, 2\}$ and $t\in [T]$, the empirical
  means of the first $t$ arm $a$ pulls is at most
  $\sqrt{\frac{2\log(T)}{t}}$ away from $\mu_a$.
\end{lemma}
