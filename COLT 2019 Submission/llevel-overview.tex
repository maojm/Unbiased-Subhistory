...
\begin{theorem}
\label{thm:llevel-1}
For any $L > 3$, there is an $L$-level disclosure policy with
regret
    $O_K\left(T^{2^{L-1}/(2^L-1)} \cdot \polylog(T) \right)$.
 In particular, there is a $O(\log\log(T))$-level recommendation
...
policy has regret
    $O_K(\min(1/\Delta, T^{1/2}) \cdot \polylog(T))$
Moreover, under this policy, each agent $t$ observes a subhistory of
size at least $\Omega( t/\polylog(T))$.
\end{theorem}

For constant number of arms, this result matches the optimal
regret rate \eqref{eq:model-OptRegret} for stochastic
bandits, up to logarithmic factors. Below we present the main techniques in our solution, and
...
agents to perform more adaptive exploration. However, we need two key modifications in the info-graph to accommodate
some new challenges. We first informally describe our techniques
...
levels below $l-1$). See Figure \ref{fig:llevel-connecting}
for the simple case with $\NG = 2$.
...
%\begin{itemize}
%\item[(i)]
(i) Consider level $(l - 1)$ and fix the last group index to be
  ...
  for $a$.
%\item[(ii)]
(ii) Every agent in level $l$ observes the sub-history from
...
\foreach \u in {1,2}
{
  ...
  \node at(-2,3){\textbf{Level $l-2$}};
  \node at(-2,5.5){\textbf{Level $l-1$}};
  \node at(-2,8){\textbf{Level $l$}};
