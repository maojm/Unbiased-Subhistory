...
\swedit{Now we provide a simple policy that can incentivize the agents
  ...
  sufficient number of times. The following Lemma follows from
  applying Chernoff bound and union bound.

}


\begin{lemma}[Concentration of arm pulls]\label{lem:t1runs}
  Consider $T_1$ indepdent runs of \ALGG of length $T_G$. Let $N_a$ be
...
\swedit{As a result, every agent in the second level will pull an arm
  with mean rewards close to the optimal, which gives us the following
  regret guarantee.}
...
% \begin{itemize}
  \paragraph{Concentration of the number of arm $a$ pulls in the first
    level.}
  \OMIT{By Lemma \ref{lem:greedy}, we know $\GdP \leq q_a \leq \GdT$.}
...
Define $W_1$ to be the intersection of all these events (i.e. $W_1 = \bigcap_{a}W_1^a$). By \Cref{lem:t1runs}, we have
\[
