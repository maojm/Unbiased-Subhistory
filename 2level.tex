...
called \emph{anonymized subhistory}. Each agent $t$ forms its estimates according to the same \emph{estimate function}: a function $f_t$ from subhistories to $[0,1]^K$, so that the estimate vector
        $(\hat{\mu}_{t,a}:\, a\in\A)$
