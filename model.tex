...
\xhdr{Unbiased sub-history policy.} The principal's goal is to reveal
...
\xhdr{Agents' behavior.} We model agents with heterogeneity in their arm selections. In particular, each agent $t$ has a private estimator that maps the revealed sub-history $H_t$ to a mean reward estimate $\hat{\mu}_a^t$ for every arm $a$, and the agent chooses the arm $a_t$ with the highest estimate. The agents' private estimators are drawn i.i.d.~from an unknown distribution. For any $t\in[T]$ and $a\in \A$, we write $N_{t, a}$ and $\bar{\mu}_a^t$ to denote the number of pulls of $a$ and the empirical mean reward of arm $a$ in sub-history $H_t$ respectively. If an arm is not pulled in $H_t$, we will let $\bar{\mu}_a^t = 0$.  We require the following assumptions.
...
\begin{remark}
  ...
  priors\swcomment{seems to be something nasty} \nicomment{need to at least say if we rule out rational agents with these assumptions (which I don't think is a disaster).}
\end{remark}
