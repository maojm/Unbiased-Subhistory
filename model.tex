...
\begin{assumption}
\label{ass:embehave}
Set constant $c_m = 1/16$.\swcomment{subscript $m$ meant anything?} There exists a constant $c_T$ such that the following is true. For each agent $t$ and arm $a$, let $t_a$ be the number of times arm $a$ has been pulled in the history observed by agent $t$. 
\begin{itemize}
\item \textbf{Close to the empirical average:} If $t_a \geq c_T$, 
\[
|\hat{\mu}^t_a - \bar{\mu}^t_a | < \frac{c_m}{\sqrt{t_a}}.
\]
\item \textbf{Initial value:} If $t_a = 0$, $\hat{\mu}^t_a$ is at least $1/3$. \swcomment{should probably mention this is just for the convenience of the analysis; mention that our analysis works as long as this is bounded away from 0?}
\end{itemize}
\end{assumption}
...
\swcomment{todo: define regret. define gap $\Delta$}
