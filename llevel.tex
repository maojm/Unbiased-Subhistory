...
\begin{corollary}
With the proper setting of $L$ and $T_1,...,T_L$ described above, the $L$-level recommendation policy gets expected regret $O(\min(polylog(T) /\Delta,  T\Delta))$. Here $\Delta = \mu_1 -\mu_2$ and the $L$-level recommendation policy does not need to know $\Delta$. Moreover, agent $t$ observes a subhistory of size at least $\Omega( \lfloor t/polylog(T)\rfloor)$. 
\end{corollary}
...
First of all, notice that even always picking the sub-optimal arm gives expected regret $T(\mu_1-\mu_2) = T\Delta$. In the rest of the proof, we just need to show the expected regret is $O(polylog(T)/\Delta)$. Now consider two cases:
\begin{itemize}
\item $\Delta < \varepsilon_{L-1} S$. In this case, we know $T\Delta = O(polylog(T)/\Delta)$. So the expected regret is $O(T\Delta) = O(polylog(T) /\Delta)$.
\item $\Delta \geq \varepsilon_{L-1} S$. In this case, we can find $l \in \{2,...,L\}$ such that $\varepsilon_{l-1} S\leq \Delta < \varepsilon_{l-2} S$. By Claim \ref{clm:l2_exploit}, we can bound the expected regret by
\begin{align*}
