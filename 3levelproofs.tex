...
\item % For each $a \in \{1,2\}$, $W_4$ implies that there exists $s_a$ such that $W_4^{s_a,a,high}$ and $W_4^{s_a,3-a,low}$ happen.
...
%   For each $s$-th first-level group and arm $a$, define
%   $\bar{\mu}_a^{1,s}$ to be the empirical mean of arm $a$ pulls in
%   this group. $W_1^{a,s}$, $W_2^{a,s,1,t}$ for
%   $ = q_a T_1- \GdT \sqrt{T_1\log(T)},...,q_a T_1- \GdT
%   \sqrt{T_1\log(T)}$ together imply that
% \[
% |\bar{\mu}_a^{1,s} - \mu_a| \leq \sqrt{\frac{2\log(T)}{q_a T_1- \GdT \sqrt{T_1\log(T)}}} \leq \sqrt{\frac{4\log(T)}{q_a T_1}}.
% \]
% The last inequality holds as long as $T$ is larger than some
% constant.
...
% For each arm $a$, define $\bar{\mu}_a$ to be the empirical mean of arm
% $a$ pulls in the first two levels. $W_1^{a,s}$ for $s=1,...,S$ and
% $W_3^{a,t}$ for $t \geq (q_a T_1- \GdT \sqrt{T_1\log(T)})S$ together
% imply that
% \[
% |\bar{\mu}_a - \mu_a| \leq \sqrt{\frac{2\log(T)}{S\left(q_a T_1- \GdT \sqrt{T_1\log(T)}\right)}} \leq \sqrt{\frac{4\log(T)}{S q_a T_1}} .
% \]
