...
(i) The number of groups does not grow with $L$ and stays as $S^2 = \Theta(\log^2(T))$ in each level. Having these groups only incurs a $\Theta(\log^2(T))$ multiplicative factor to the expected regret.\\
