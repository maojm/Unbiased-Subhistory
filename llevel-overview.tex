...
the ``lucky'' property, it suffices to have the agents in different
$l$-th level groups to observe disjoint sub-histories that takes place
in level $(l - 1)$. Moreover, as long as the group size in levels
lower than $(l-1)$ are substantially smaller than group size of level
$l-1$, the ``lucky'' property does not break even if different groups
in level $l$ observe overlapping sub-history from levels
$\{1, \ldots, l-2\}$.
...
\OMIT{In our 3-level policy, the second level has
  ...
  $l \in \{3,...,L-1\}$, we do want agents in different $l$-th level
...
(iii) For each set of groups in level $l-1$, agents in level $l$ see
