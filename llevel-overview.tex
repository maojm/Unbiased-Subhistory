...
\end{theorem}
\swcomment{think the theorem statement implies independence of
  $\Delta$}

In this section, we present the main techniques in our solution, and the 
full proofs of Theorem \ref{thm:llevel-1} and Theorem \ref{thm:llevel-2} will be deferred to Appendix \ref{sec:llevel-details}. Similarly as Section \ref{sec:3level}, we first prove them in the case of 2 arms (Theorem \ref{thm:llevel} and Corollary \ref{cor:llevel}). We then extend them to the case of constant number of arms (Theorem \ref{thm:constarm}).

We will now present the main idea of extending from 3-level to
...
\xhdr{Connecting structures between levels.} In our 3-level
policy, the second level has $\NG = \Theta(\log(T))$ groups. We do so to
...
For each level, our $L$-level policy has $\NG^2$ groups for some $\NG = \Theta(\log(T))$. The groups in the $l$-th level are labeled as $G_{l,u,v}$ for $u,v\in[\NG]$. For any $l \in \{2,...,L\}$ and $u,v,w\in [\NG]$, agents in group $G_{l,u,v}$ see the history of agents in group $G_{l-1,v,w}$ (and by transitivity all agents in levels below $l-1$). See Figure \ref{fig:llevel-connecting} for a graphical view (as space limit, we only show 3 levels and 4 groups in each level). This connecting structure has three nice properties: \\
(i) The number of groups does not grow with $L$ and stays as $\NG^2 = \Theta(\log^2(T))$ in each level. Having these groups only incurs a $\Theta(\log^2(T))$ multiplicative factor to the regret.\\
(ii) Consider groups $G_{l-1,1,v},G_{l-1,2,v},...,G_{l-1,\NG,v}$ (e.g. $G_{l-1,1,1}$ and $G_{l-1,2,1}$ with red circles in the figure ) as a set of groups indexed by $l-1$ and $v$. The agents in a set of groups see the same history. They get ``lucky'' for some arm at the same time.\\
