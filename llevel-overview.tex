...
visualization of simple case with $\sigma = 2$). Two observations are
in order:
...
\item(i) For any level $l \geq 2$, fix the last group index to be $v$, and consider the groups $G_{l-1,1,v},G_{l-1,2,v},...,G_{l-1,\NG,v}$ (e.g. $G_{l-1,1,1}$ and $G_{l-1,2,1}$ circled in the Figure \ref{fig:llevel-connecting}). The agents in a set of groups see the same history. They get ``lucky'' for some arm at the same time.\\
\item(ii) For each set of groups in level $l-1$, agents in level $l$ see
