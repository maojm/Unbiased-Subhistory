...
\item(i) For any level $l \geq 2$, fix the last group index to be $v$, and consider the set of  groups $\{G_{l-1,i,v}\mid i \in [\NG]\}$ (e.g. $G_{l-1,1,1}$ and $G_{l-1,2,1}$ circled in the Figure \ref{fig:llevel-connecting}). The agents in a set of groups see the same history. They get ``lucky'' for some arm at the same time.\\
