...
\begin{proof}[Proof of Lemma~\ref{lem:greedy}]
  ...
  \]
  %\nicomment{Wait, where did we set $\estN$?} 
...
$W^{a,N}_2$ to be the event that the empirical mean of the first $N$
realized rewards in the tape of arm $a$ is at most
$\sqrt{\frac{2\log(T)}{N}}$ away from $\mu_a$. Define $W_2$ to be the
intersection of these events (i.e.  $\bigcap_{a,N\in[T]} W^{a,N}_2$).  By
Chernoff bound,%\swcomment{$t$ may be confusing here}
\[
\Pr[W^{a,N}_2] \geq 1 - 2\exp(-4\log(T)) \geq 1-2/T^4.
\]
By union bound,
\[
